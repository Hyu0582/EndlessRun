...
public class ShurikenSpawner : MonoBehaviour
{
    ...
    void SpawnShuriken(SpawnModeConfig config)
    {
        GameObject shurikenPrefab = prefabObstacles[Random.Range(0, prefabObstacles.Length)];
        float randomY = Random.Range(config.minY, config.maxY);
        if (config.mode == SpawnMode.Normal)
        {
            // Spawn bình thường: 1 shuriken ngẫu nhiên
            Vector2 spawnPosition = new Vector2(config.spawnX, randomY);
            Instantiate(shurikenPrefab, spawnPosition, Quaternion.identity);
        }
        else if (config.mode == SpawnMode.Wave)
        {
            // Spawn 3 shuriken theo hình /
            float spacing = (config.maxY - config.minY) / (config.spawnCount * 3f); // Khoảng cách Y giữa các shuriken
